...
@Component({
  ...
  styles: [
    `
      ...
      /* Fix child component host elements not stretching */
      app-sidebar {
        display: flex;
        flex-direction: column;
        height: 100vh;
...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  // Create Room (temporary until Room Service is built)
  handleCreateRoom() {
  ...
  // Proper Logout with Redis Blacklist
  handleLogout() {
    this.socketService.disconnect();
    ...
    this.authService.logout().subscribe({
      ...
      error: (err) => {
        ...
        // still clear localStorage for safety
        localStorage.clear();
        this.router.navigate(['/login']);
      },
    });
